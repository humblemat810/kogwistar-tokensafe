...
# Assign a realm role to a bundled Keycloak service-account client.
# This is idempotent and safe to rerun. It is used by local compose and by the
# production rehearsal runner so the requested bearer-token path is actually usable.
...
ROLE_CLIENT_ID="${MODELKEYGUARD_KEYCLOAK_ROLE_GRANT_CLIENT_ID:-${MODELKEYGUARD_OIDC_ADMIN_CLIENT_ID:-modelguard-admin}}"
REQUIRED_ROLE="${MODELKEYGUARD_KEYCLOAK_ROLE_GRANT_ROLE:-${MODELKEYGUARD_ADMIN_REQUIRED_ROLE:-model.admin}}"
export KEYCLOAK_URL KEYCLOAK_REALM BOOTSTRAP_USER BOOTSTRAP_PASSWORD ROLE_CLIENT_ID REQUIRED_ROLE
...
role_client_id = os.environ["ROLE_CLIENT_ID"]
...
clients = request_json("GET", f"{base}/admin/realms/{realm}/clients?clientId={urllib.parse.quote(role_client_id, safe='')}", token=token)
if not clients:
    raise SystemExit(f"Keycloak client not found: {role_client_id}")
...
if not service_user or not service_user.get("id"):
    raise SystemExit(f"service account user not found for client: {role_client_id}")
...
print(f"Assigned realm role {required_role} to service account client {role_client_id}")
PY
