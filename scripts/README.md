...
| `deploy_remote_stack.sh` | SSH-based deploy wrapper for a same-machine lower-privilege user or another host. It builds the gateway image locally, loads it on the remote Docker host, stages secrets into remote tmpfs for the lifetime of the run, and then runs the local production or gateway-only workflow remotely. |
